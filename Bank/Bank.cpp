...
enum enumOperationTransaction { Depsite = 1, Withdraw = 2, Transfer = 3, TotalBalance = 4, GoBack = 5 };
...
string ConvertRecordUserToLineString(Users user, string delim = "#//#")
{
	...
	Record = Record + RoleType(user.role);
	return  Record;
...
void SingUsers() {
	...
	case SignOption::login: GoToMenue(GetRole());
		break;
...
void HeaderOperation(int choice)
{
	...
	case 5:cout << right << setw(70) << " | Page  Search  Client |" << "\n\n\n";
		break;
	case 6:cout << right << setw(70) << " | Page SearchAction |" << "\n\n\n";
		break;
		...
		if (FindClientByAccountNumber(AccountNumber, vclient, client))
		{
			...
			for (Client& client : vclient) {
				if (client.AccountNumber == AccountNumber) {
					...
					cin >> amount;
					DataAction(action, client, amount, Operation);
...
void WithdrawClient(enumOperationTransaction Operation)
{
	string AccountNumber;
		...
		if (FindClientByAccountNumber(AccountNumber, vclient, client))
		{
			...
			for (Client& client : vclient) {
				if (client.AccountNumber == AccountNumber) {
					cout << "Enter Amount To Deposit From Your Balance? " << endl;
					cin >> amount;
					while (ValidateAmoun(client.Balance, amount)) {
...
}void AdminManue()
{
	...
	cout << right << setw(10) << "[6]" << "SearchAction" << endl;
	cout << right << setw(10) << "[7]" << "Logout" << endl;
	cout << right << setw(10) << "[8]" << "Exit" << endl;
	cout <<
		"\n_______________________________________________________\n";
